...
class Data {
    ...
    campusToColor(campus) {
        return data
            .campuses
            .filter(x => x.name[x.name.length - 1] == campus)[0]
            .color;
    }

    extendCategories(categories) {
        categories.forEach(category => {
            category.places = this
                .places
                .filter(place => place.category == category.id);
        });
    }
    ...
    static isFunded(searched, element) {
        ...
        return element
            .name
            .toLowerCase()
            .search(searched) != -1 || element
            .short
            .toLowerCase()
            .search(searched) != -1 || tmp1 || tmp2;
    }
}
...
class Modal {
    ...
    static initModalInfoBuilding(element) {
        ...
        modal.innerHTML = `<div class='mui-container mui-panel'>` + `<h1>${View.getShort(element)}${element.name}</h1>` + ` <dl>${View.getElement(element, "address", "Adres")}
                ...
                ${View.getElement(element, "tags", "Tagi")}</dl>` + `<div class="mui-row mui--text-center">` + `<button class="mui-btn" onclick="view.overlayOff()"><i class="fa fa-close"></i> Zamknij</button>` + `</div>` + `</div>`;
        return modal;
    }
...
class View {
    ...
    static getCategory(element) {
        if (element.category) {
            return "<dt>Kategoria</dt><dd>" + data
                .categories
                .filter(x => {
    ...
    static getElementLink(element, propertyName, propertText) {
        return element[propertyName]
            ? `<dt>${propertText}</dt><dd><a href="${element[propertyName]}" target="_blank">Strona WWW</a></dd>`
            : ``;
    }
    ...
    static getPlaces(element) {
        let placess = data
            .places
            .filter(x => x.building.split(",").some(y => y == element.id));
        if (placess.length > 0) {
            return "<dt>Jednostki w budynku</dt><dd>" + placess.reduce((x, y) => x + `<a href="?placeId=${y.id}">${y.name}</a><br>`, ``) + "</dd>";
        } else {
            return ``;
        }
    }
    ...
    printBuildings(buildings) {
        return `<strong onclick='View.toggleListElement(this);'> <span><i class="fa fa-building"></i> Budynki</span>${View.arrowSpan()}</strong>` + `<ul style='display:none;'>` + buildings.reduce((a, building) => a + View.prepareLink(building), "") + `${View.arrowSpan()}</ul>`;
    }

    printCategories(categories1) {
        return categories1.reduce((a, category) => a + (!category.isSubCat
            ? `<li>${this.printCategory(category)}</li>`
            : ``), ``);
    }
    ...
    prepareUpdateMarker(id, isPlace = false) {
        if (isPlace) {
            ...
            let building = data
                .buildings
                .filter(building => building.id == id)[0];
    ...
    palceModal(element) {
        ...
        let tmp = idBuildings.map(() => {
            let result = idBuildings.reduce((a, x) => {
                ...
                return a + `<div class="mui-divider"></div><a href='javascript:view.setMarkerCloseModal(${JSON.stringify({
                    id: element.id,
                    name: element.name})},${index})'>${tmp.name}</a><br><p>${tmp.address}</p>`
                }, "");
                index = -1;
                return result;
            });
            ...
            let next = tmp.map(x => {
                index += 1;
                return `<strong>${element.name}</strong>` + `<p>Wybierz jeden z budynków tej jednostki:</p>${x}`;
            });

            let modal = document.createElement('div');
            modal.innerHTML = `<div class='mui-container mui--text-center mui-panel'><div class="mui--text-center">${next[0]}</div>` + `<button class="mui-btn" onclick="view.overlayOff()"><i class="fa fa-close"></i> Zamknij</button>` + `</div>`;
        ...
        prepareInfoContent(element, isPlace, normal = true) {
            if (isPlace) {
                ...
                if (idBuildings.length > 1) {
                    ...
                    return [`<p><strong>${element.name}</strong><br>` + `${building.name}<br>` + `${View.getAddresExt(building)}` + `</br><a href='javascript:view.activateModalInfo(${element.id},true);'>Więcej informacji</a>`];
                }
            } else {
                if (normal) {
                    QueryHelper.UpdateURL(element.name, "?buildingId=" + element.id);
        ...
        showSidedrawer() {
            ...
            mui.overlay('on', {
                onclose: () => {
                    this.sidedrawerElement.className = this
                        .sidedrawerElement
                        .className
                        .replace(' active', '');
                    document
    ...
    class MapsApi {
        constructor(zoom, initCoordinate) {
            ...
            let googleSat = L.tileLayer('http://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', {
                    ...
                    attribution: '&copy;<a href="https://www.google.com/intl/en_en/help/terms_maps.html">Google la' +
                ...
                full = L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'});
            ...
            let layerControl = L
                .control
                .groupedLayers(baseLayers, overlaysL, options);
            this
                .map
            ...
            L
                .control
                .fullscreen({position: 'topleft', forceSeparateButton: true, forcePseudoFullscreen: true, fullscreenElement: false})
                .addTo(this.map);

            this
                .map
                .on('enterFullscreen', MapsApi.enterFullscreen);
            this
                .map
                .on('exitFullscreen', MapsApi.exitFullscreen);
        ...
        setCenter(latLng) {
            this
                .map
                .panTo(new L.LatLng(latLng[0], latLng[1]));
        }
    ...
    function init() {
        let buildings,
            ...
            places,
            campuses;
        JSONHelper.loadJSON('json/categories.json', (categories) => {
            JSONHelper.loadJSON('json/places.json', (places) => {
                JSONHelper.loadJSON('json/buildings.json', (buildings) => {
                    JSONHelper.loadJSON('json/campuses.json', (campuses) => {
    ...
    Array
        .prototype
        .forEach
        .call(document.querySelectorAll('.clearable-input>[data-clear-input]'), function (el) {
            el
